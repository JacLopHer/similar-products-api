...
class LoadSimilarProductIdsAdapterTest {
    ...
    void shouldLoadSimilarProductIds() {
        ...
        when(productApiClient.getSimilarProductIds("1")).thenReturn(Mono.just(apiIds));

        List<ProductId> result = adapter.loadSimilarProductIds(productId).block();
    ...
    void shouldReturnEmptyListWhenNoSimilarProducts() {
        ...
        when(productApiClient.getSimilarProductIds("999")).thenReturn(Mono.just(List.of()));
